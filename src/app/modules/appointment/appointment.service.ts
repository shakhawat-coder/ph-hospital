...
const cancelUnpaidAppointments = async () => {
  ...
  if (unpaidAppointments.length > 0) {
    console.log(`Found ${unpaidAppointments.length} unpaid appointments to cancel.`);
  } else {
    // console.log("No unpaid appointments found to cancel.");
    return;
  }
  ...
  await prisma.$transaction(async (tx) => {
    const cancelCount = await tx.appointment.updateMany({
      where: {
        id: {
    ...
    console.log(`Successfully cancelled ${cancelCount.count} appointments.`);
